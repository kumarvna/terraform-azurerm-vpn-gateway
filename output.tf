...
output "vpn_gateway_public_ip" {
  ...
  value       = flatten(concat(azurerm_public_ip.pip_gw.ip_address, var.enable_active_active ? azurerm_public_ip.pip_aa.ip_address : null))
}

output "vpn_gateway_public_ip_fqdn" {
  description = "Fully qualified domain name of the virtual network gateway"
  value       = flatten(concat(azurerm_public_ip.pip_gw.fqdn, var.enable_active_active ? azurerm_public_ip.pip_aa.fqdn : null))
}
